...
def create_metadata_json(file_name, file_size, file_hash, compression_enabled, compression_level, chunk_size, total_chunks):
    """
    创建元信息JSON字符串
    """
    # 将压缩算法与压缩等级融合为一个字段
...
def split_data_with_metadata(data, chunk_size, file_name, file_hash, compression_enabled=False, compression_level=-1):
    """
    ...
    for i in range(num_chunks):
        ...
        header = f"{i}/{chunk_size}/{total_size}|".encode('ascii')
...
def create_qr_pdf_from_file(input_file, output_file, chunk_size=2100, img_size=400,
    ...
    chunks = split_data_with_metadata(file_data, chunk_size, file_name, file_hash, compression_enabled, compression_level)
