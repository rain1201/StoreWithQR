...
def split_data_with_metadata(data, chunk_size, file_name, file_hash, compression_enabled=False, compression_level=-1,version=2,hash=""):
    """
    将二进制数据切片，第一个二维码保存元信息 JSON，其余保存文件数据。
    元信息格式：JSON 字符串包含文件名、压缩、hash、分块等信息
    数据块格式 (二进制): b'index/chunk_size/total_size|payload'
    """
    total_size = len(data)
    num_chunks = math.ceil(total_size / chunk_size)

    # 创建元信息 JSON 作为第一个二维码的内容
    ...
    # 从索引 1 开始添加数据块 (保持与原来相同的格式，但索引从 1 开始对应实际数据)
    ...
    for i in range(num_chunks):
        ...
        if compression_enabled and compression_level > 0:
            ...
            if i == 0:
                print(f"压缩前总大小：{total_original_size} 字节")
            print(f"块 {i+1}: {original_payload_size} -> {compressed_payload_size} 字节 ({compressed_payload_size/original_payload_size:.2%})")

        # 构建二进制头部："索引/块大小/总大小|" (这里索引从 1 开始，因为 0 是元信息)
...
def detect_encoding(file_data):
    ...
    # 首先检查是否为纯文本文件
    try:
        # 尝试以 ASCII 解码，看是否是纯 ASCII 文本
        file_data.decode('ascii')
        return 'ASCII'
    except UnicodeDecodeError:
        # 如果不是纯 ASCII，使用 chardet 库检测编码
...
def create_qr_pdf_from_file(input_file, output_file, chunk_size=2100, img_size=400,
    ...
    print(f"PDF 设置 - 纸张：{paper_size}{orientation}, 布局：{cols}x{rows}, 二维码大小：{qr_size}mm")
    ...
    if compression_enabled:
        ...
        print(f"启用压缩：ZSTD 级别 {compression_level} (每个块单独压缩)")
    else:
    ...
    print(f"切分为 {len(chunks)} 个数据块 (包括 1 个元信息块)")
    ...
    for i, chunk in enumerate(chunks):
        ...
        if i == 0:
            ...
            # 对于数据块，解析出原始索引（因为现在索引从 1 开始对应实际数据）
